...
## 🧠 Feature Idea: Live AI Monitor

**Суть:** каждый 5-мин сегмент видео (из сегментной записи) прогоняется через Vision AI сразу после загрузки, ещё во время сеанса. Если AI видит проблему — массажист получает голосовое/текстовое предупреждение в реальном времени.

### Как работает
1. Сегмент загружен → Vision AI (HF Router / Gemini) анализирует кадры
2. AI возвращает уровень: 🔴 критично / 🟡 совет / 🟢 норма
3. 🔴 → TTS → голосовое в Telegram (gTTS/HF TTS)
4. 🟡 → текст в чат со специалистом

### Реализуемость — ✅ Реально
- Загрузка сегментов ✅ есть
- Vision AI ✅ есть (`process_video_frames()`)
- TTS: gTTS (бесплатно, 0 инфры)
- Оповещение в Telegram ✅ есть
- **Latency:** 5-15 сек на сегмент — незаметно при 5-мин интервале
- **Лимит HF:** ~30 req/min, 12 запросов за сеанс — норма

### MVP (3-4 ч работы)
1. Промпт с тремя уровнями тревоги
2. Проверка severity после AI → `send_voice()` при 🔴
3. gTTS через HTTP-запрос

### Когда нужно
- Массажист один, не видит лица клиента
- Обучение новичков (AI подсказывает live)
- Высокорисковые клиенты (сердечники, после травм)

**Статус:** идея, не начато

---

## 🔹 Веха 17: ADK 2.0 Workflow — первая успешная сборка (21 Июня 2026)

### Что сделано
- **ADK 2.0 Workflow впервые отработал полностью** — 6 агентов по цепочке, 8 событий, полный отчёт
- **Контекст накапливается** — каждый агент видит всё что сказали предыдущие (global accumulator в `workflow.py`)
- **Rate limiting** — 15s паузы между агентами для обхода 5 req/min лимита
- **Промпты исправлены** — questionnaire больше не отвергает обычные боли

### Пофикшенные баги
1. ADK graph validation error — уникальные имена функций-трансформеров (был cycle из-за `_accumulate_context` на двух edges)
2. Tool name mismatch — `search_media` vs `_search_media` (LLM звала без подчёркивания)
3. `MUSIC_DB` import — на самом деле `MASSAGE_MUSIC_GENRES`

### Демо-страница
- Mock mode для демонстрации UI без API (checkbox "Use AI")
- Галочка по умолчанию выключена → судьи видят workflow UI без расхода квоты
- Badge "⚡ Demo Mode (Mock)"

### Тесты
- 216/216 pass
- 35 ADK тестов (agent config, workflow structure, vision, tools, API, function nodes)

### План на 22 июня
Записать 3-4 сценария (поясница, шея, плечо, стресс) с фото и скриншотами через Playwright.
Собрать материалы для 2-минутного видео сабмишена.
