...
- **Победа над Конфликтами**: Решена проблема `TelegramConflictError` через принудительный сброс очереди сообщений (`drop_pending_updates=True`). Бот научился «просыпаться» в одиночестве, не мешая своим прошлым воплощениям.
- **Интерфейс Настроек**: В меню добавлена кнопка «⚙️ Настройки». Теперь пользователь может вручную выбирать Оракула (Gemini, HF или Авто).
- **Великая Миграция HF**: Обнаружено полное отключение старого эндпоинта `api-inference` (ошибка 410 Gone). Весь трафик перенаправлен на современный `router.huggingface.co`.
