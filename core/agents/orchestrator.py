...
class MassageConsultationOrchestrator:
    ...
    def _build_context(self, target_id: str, questionnaire: str, results: dict, include_all=False) -> str:
        ...
        hints = {
            ...
            "final_expert": "Синтезируй все данные и выдай ОКОНЧАТЕЛЬНОЕ ЗАКЛЮЧЕНИЕ по формату. Обязательно укажи пункт 6) СЕАНСЫ (рекомендуемая длительность в минутах) и пункт 7) МУЗЫКА (жанр и количество треков для плейлиста).",
        }
        parts.append(f"\n\n{hints.get(target_id, '')}")
        return "\n".join(parts)
