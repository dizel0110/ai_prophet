...
class SpecialistFactory:
    ...
    def create(cls, chat_id: int, role_description: str, name: Optional[str] = None) -> Optional[DynamicSpecialist]:
        ...
        prompt = (
            f"Создай персонального консультанта по описанию: \"{role_description}\"\n\n"
            f"Верни JSON с полями:\n"
            f"  name — короткое имя специалиста (2-4 слова, рус.)\n"
            f"  role — его профессиональная роль (1 предложение)\n"
            f"  skills — ключевые навыки (3-5 пунктов через запятую)\n"
            f"  system_prompt — подробная инструкция для ИИ (на рус., 5-10 предложений):\n"
            f"    • кто он, его опыт и экспертиза\n"
            f"    • какие вопросы задавать клиенту для сбора информации\n"
            f"    • как направлять клиента в массажный салон AI Prophet\n"
            f"    • что НЕЛЬЗЯ делать (ставить диагнозы, назначать лекарства)\n"
            f"    • стиль общения (профессиональный, заботливый, понятный)\n\n"
            f"Пример для \"эксперт по стоун-терапии\":\n"
            f'{{"name": "Мастер стоун-терапии", "role": "Специалист по массажу горячими камнями", "skills": "стоун-терапия, терморегуляция, расслабление мышц, работа с энергетическими центрами", "system_prompt": "Ты — Мастер стоун-терапии с 8-летним опытом. Узнай у клиента: есть ли проблемы с давлением, кожей, варикозом. Рекомендуй стоун-терапию при мышечном напряжении, стрессе. Не назначай медицинских процедур. Общайся спокойно и уверенно."}}\n\n"
            f"Ответь JSON: {{\"name\": \"...\", \"role\": \"...\", \"skills\": \"...\", \"system_prompt\": \"...\"}}"
        )
        ...
        try:
            ...
            specialist = DynamicSpecialist(
                ...
                system_prompt=f"Ты — {name or role_description[:30]}. Твоя роль: {role_description}. Консультируй клиента, задавай вопросы, направляй в массажный салон. Не ставь диагнозов.",
                skills="",
                created_at=time.time(),
                chat_id=chat_id,
            )
