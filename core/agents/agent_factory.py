...
from config import GEMINI_KEY, HF_TOKEN, HF_TASKS, FALLBACK_MODELS, get_vertical_name
...
def _get_universal_consultant_prompt(user_mode: str = "vertical") -> str:
    platform = get_vertical_name() if user_mode == "vertical" else "AI Prophet"
    return (
        f"Ты — Мастер-консультант, главный гид по платформе «{platform}» и массажному салону.\n\n"
        "Твоя экспертиза:\n"
...
UNIVERSAL_CONSULTANT_SYSTEM_PROMPT = _get_universal_consultant_prompt("vertical")
...
class SpecialistFactory:
    ...
    @classmethod
    def create(cls, chat_id: int, role_description: str, name: Optional[str] = None, user_mode: str = "vertical") -> Optional[DynamicSpecialist]:
        if name == UNIVERSAL_CONSULTANT_NAME or role_description == UNIVERSAL_CONSULTANT_ROLE:
            specialist = DynamicSpecialist(
                ...
                system_prompt=_get_universal_consultant_prompt(user_mode),
        ...
        prompt_parts.append('Верни JSON с полями:\n'
            ...
            f'    • как направлять клиента в {get_vertical_name()}\n'
        ...
        try:
            ...
            specialist = DynamicSpecialist(
                ...
                system_prompt=sp if sp else f"Ты — {data.get('name', role_description[:30])}. {data.get('role', role_description)}. Навыки: {data.get('skills', '')}. Консультируй клиента по своей специализации, задавай уточняющие вопросы, направляй в {get_vertical_name()}. Не ставь медицинских диагнозов. Общайся профессионально и заботливо. Отвечай на языке пользователя. По умолчанию — русский. НЕ используй китайские иероглифы.",
