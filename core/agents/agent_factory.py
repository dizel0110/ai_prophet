...
ВАЖНО: ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ ЯЗЫКЕ. НИКАКИХ ИЕРОГЛИФОВ."""
...
BUILT_IN_AGENTS = [
    ...
    },
    {
        ...
        "system_prompt": "Ты — Эксперт по техникам массажа. На основе данных о клиенте (жалобы, здоровье, предпочтения) ты рекомендуешь конкретные техники массажа. Техники: классический, спортивный, лимфодренажный, антицеллюлитный, точечный, миофасциальный, стоун-терапия, тайский, баночный, медовый. Для каждой указываешь показания и ожидаемый эффект. Консультируй клиента, помогай выбрать подходящую технику. НЕ ставь диагнозов и НЕ назначай медикаментов. ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ. НИКАКИХ ИЕРОГЛИФОВ.",
...
class SpecialistFactory:
    ...
    def create(cls, chat_id: int, role_description: str, name: Optional[str] = None) -> Optional[DynamicSpecialist]:
        ...
        system = "Ты — создатель экспертов. Отвечай только в формате JSON. ТОЛЬКО НА РУССКОМ. НИКАКИХ ИЕРОГЛИФОВ."
        ...
        try:
            ...
            specialist = DynamicSpecialist(
                ...
                system_prompt=f"Ты — {fallback_name}. Твоя роль: {fallback_role}. Консультируй клиента, задавай вопросы, направляй в массажный салон. Не ставь диагнозов. ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ. НИКАКИХ ИЕРОГЛИФОВ.",
                skills="",
                created_at=time.time(),
                chat_id=chat_id,
            )
