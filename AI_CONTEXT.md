...
4. **Голосовые сообщения с подтверждением** (Февраль 2026):
   - После транскрибации показывается текст
   - Кнопки: "✅ Выполнить" / "✏️ Редактировать" / "❌ Отмена"
   - HF Whisper: `openai/whisper-large-v3-turbo` (v3 удалена)

### ⏳ Ожидает тестирования:
- Отправка плейлистов (3-5 треков подряд)
- Обработка ошибок для длинных треков (30+ мин)
- Транскрибация через HF Whisper (новая модель)
