...
AI Prophet — это **платформа-конструктор**, а не один сервис. Основа — ИИ-проводник (Мастер-консультант). Всё остальное — **вертикали**, каждая из которых обслуживает конкретную сферу бизнеса.

### Что такое вертикаль
Вертикаль = **клон бота + Mini App** под новую сферу, с:
- Своей анкетой (`config/questionnaire_steps.json`)
- Своими агентами (`core/agents/registry.py`)
- Своей музыкальной базой (`music_db.py`)
- Своими промптами (system prompt Мастер-консультанта)
- Своим обучением (`config/education_*.md`)
- Своей админ-панелью

**Первая вертикаль:** массажный салон 🖐
**Следующие:** фитнес-клуб, юридическая консультация, медицинский центр, образование

### Принцип клонирования
Новая вертикаль = новый инстанс бота (новый `TELEGRAM_TOKEN`). Кодовая база общая, конфигурация разная. Изменения в ядре (`core/`, `handlers/`, `main.py`) автоматически идут во все вертикали.

## 🎓 Система обучения (Education)

Встроенная обучающая платформа внутри Mini App. Вкладка **🎓 Обучение** доступна всем ролям.

### Как работает
1. Контент хранится в `config/education_*.md` (markdown, текстовые файлы — проходят git/HF Spaces)
2. `GET /api/education/{role}` — возвращает markdown для client / masseur / admin
3. Mini App рендерит через `renderMarkdown()` (bold, code, links, списки)
4. Три роли переключаются кнопками `👤 Клиент / 🖐 Массажист / ⚙️ Админ`

### Файлы обучения
- `config/education_client.md` — что такое AI Prophet, как пользоваться ботом, как проходит консультация, FAQ
- `config/education_masseur.md` — как читать сводку, дневник, интерпретировать AI-рекомендации
- `config/education_admin.md` — управление клиентами, статистика, тестовые пациенты, команды

### 🎬 Видео в обучении
В `education_*.md` можно встраивать видео двумя способами:
- `[YOUTUBE: video_id]` — встроенный YouTube-плеер (responsive, 16:9)
- `[VIDEO: https://...]` — прямая ссылка на MP4-файл

Рендерятся через `renderMarkdown()` в Mini App. YouTube — через iframe, MP4 — через `<video controls>`.
...
### 🔥 Правило: Education in Sync (EIS)
Любое изменение платформы, затрагивающее пользовательский опыт, **должно сопровождаться обновлением соответствующих education-файлов**. Это не опционально.

- **Новая фича** → описать в education_client.md (как пользоваться) + education_masseur.md (как интерпретировать) + education_admin.md (как управлять)
- **Изменение UI** → обновить скриншоты/описание
- **Новая команда бота** → добавить в таблицу команд
- **Новый тип данных в админке** → описать в education_admin.md

Проверка перед коммитом: `git diff --name-only HEAD` → если меняются файлы из списка ниже, нужно обновить education:
```
handlers/*.py, main.py, static/massage/index.html, core/*.py, config/questionnaire_steps.json
```

### 🧪 Тестовые пациенты
Админ может создавать тестовых пациентов для демонстрации и обучения:
- `POST /api/admin/test_client` — генерирует профиль + 1-4 консультации
- `POST /api/admin/test_client/delete` — удаляет
- В Mini App: кнопка «🧪 Создать тестового» + «🗑 Удалить»
- Тестовые помечены бейджем `🧪 Тест`, серым цветом, телефон `+7000...`
- **Не влияют на реальную статистику** — `get_stats(filter_test=True)` по умолчанию
...
### 🔥 Правило: Client List показывает всех
Админ-панель (`/api/admin/clients`) возвращает **всех пользователей**, не только с анкетой. Пользователи без анкеты имеют `has_questionnaire: false` и бейдж `⚠️ Нет анкеты` в списке. Сортировка: сначала с анкетой, потом без.

Кнопка записи видео различается в зависимости от анкеты:
- **есть анкета** → `🎥 Начать сеанс массажа` — реальная работа с клиентом, видео в Telegram + запись в дневник
- **нет анкеты** → `🎬 Тренировка / тест системы` — тест/тренировка, видео в Telegram без дневника

Без анкеты предсеансовая сводка не формируется, запись не сохраняется в дневник.
...
| Зона | Цель | Правило |
|------|------|---------|
| `core/*.py` (бизнес-логика) | **≥70%** | Новый модуль → тест. Багфикс → тест если причина не в хендлере |
| `handlers/*.py` | **≥30%** (опционально) | Только сложная логика (state machine, валидация). Чистые вызовы API/TG — не тестировать |
| `main.py` (API) | **≥20%** | Только эндпоинты со сложной логикой (`/book`, `/analyze`) |
| `static/` (HTML/JS) | **не измеряем** | Нет рантайма для тестов |

> **Приоритет:** критические цепочки (бронь → уведомление → подтверждение) и data-driven логика (анкета, профили) в первую очередь.

**Правило создания:** любой новый модуль в `core/` или новая существенная логика (не багфикс строки на 2) должна сопровождаться тестом. Исключение — изменения UI/хендлеров (Telegram, HTML).
...
### Кнопка «Мигрировать схему БД» — для чего

| Сценарий | Нужна ли миграция схемы? |
|----------|------------------------|
| Таблицы созданы старой версией кода, не хватает колонок | **Да** — запускает ALTER TABLE ADD COLUMN IF NOT EXISTS |
| Таблиц ещё нет, Space запускается впервые | **Нет** — `init_schema()` создаёт таблицы со всеми колонками |
| После обновления кода (новые колонки в SCHEMA_SQL) | **Нет** — `init_schema()` выполнит ALTER TABLE при старте через `SUPABASE_DB_URL` |
| `SUPABASE_DB_URL` не задан | **Не работает** — кнопка покажет ошибку |

Кнопка — это **ручной триггер** для случая, когда не хочешь ждать рестарта Space.
...
Для приоритизации все фичи классифицируются по степени влияния на бизнес:

| Класс | Название | Описание | Примеры |
|-------|----------|----------|---------|
| **🛡️ Critical Reliability** | Надёжность и доверие | Баги/пробелы, подрывающие доверие клиентов | Автоотмена записей, доставка уведомлений, дуал-райт |
| **🚀 Core Value** | Основная ценность | Фичи, ради которых пользователь приходит | AI-консультация, бронирование, анкета |
| **✨ Experience** | Улучшение опыта | Приятные дополнения, ускоряющие работу | Цветные слоты, утренний дайджест, история записей |
| **🔮 Future** | Перспективные | Требуют проработки, могут изменить рынок | Client-Side AI, автоподтверждение, интеграция с CRM |

### 🛡️ Critical Reliability — что всегда держать в голове

Фичи этого класса **нельзя откладывать**. Если что-то не доделано — система ненадёжна:

- **Автоотмена зависших записей**: клиент создал запись → массажист не ответил → клиент висит в неопределённости. Решение: reminder за 3ч, автоотмена за 1ч до сеанса.
- **Доставка уведомлений**: массажист должен гарантированно получить уведомление. Fallback: утренний дайджест + напоминание.
- **Отказоустойчивость хранилища**: dual-write (JSON + Supabase). Если Supabase лежит — JSON сохраняет данные.
- **Валидация на сервере**: клиент не может обойти фронтенд-ограничения (is_first_visit, лимит pending, service gate).
- **Логирование сбоев**: любая ошибка уведомления / записи логируется с контекстом.

### Автоотмена записей (Critical Reliability)

**Логика:**
```
Создана pending запись → массажисту уведомление с ✅❌
         ↓
   +3ч  → напоминание массажисту (если всё ещё pending)
         ↓
   +1ч  → 🚫 АВТООТМЕНА → клиенту "❌ Запись отменена"
                           → массажисту "❌ Автоотмена"
                           → слот освобождается
```

**Утренний дайджест:** в 09:00 каждому массажисту — сводка всех pending записей.

**Фоновый таск:** `asyncio.create_task(booking_housekeeping())` в `start_bot_polling()`, проверка каждые 60 секунд.

**Конфигурация (хардкод, можно вынести в env):**
| Параметр | Значение | Смысл |
|----------|----------|-------|
| `AUTO_CANCEL_MINUTES` | 60 | За сколько минут до сеанса автоотмена |
| `REMINDER_WINDOW` | 175-190 мин | Окно для напоминания (~3ч) |
| `MORNING_DIGEST_TIME` | 09:00 | Время утреннего дайджеста |
